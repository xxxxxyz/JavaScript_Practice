...
// axios 公共配置
// 基地址
(external_axios_default()).defaults.baseURL = "http://geek.itheima.net";

// 添加请求拦截器
external_axios_default().interceptors.request.use(
  function (config) {
    // 在发送请求之前做些什么
...
// 添加响应拦截器
external_axios_default().interceptors.response.use(
  function (response) {
    // 2xx 范围内的状态码都会触发该函数。
...
/* harmony default export */ const request = ((external_axios_default()));
...
theImg.src = logo_namespaceObject;
